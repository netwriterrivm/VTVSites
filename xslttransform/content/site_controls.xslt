...
<xsl:stylesheet version="1.0" exclude-result-prefixes="asp ucidb" xmlns:ucidb="http://www.piet.nl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:asp="urn:schemas-microsoft-com:datatypes">
  ...
  <xsl:param name="content_fragment_type">site_controls</xsl:param>
  ...
  <xsl:include href="site_controls.common.xslt"/>
  ...
  <xsl:include href="site_controls_included.xslt" />
    ...
    <xsl:choose>
      <xsl:when test="$nodeName!='' and $XlinkId!=''">
        <xsl:choose>
          ...
          <xsl:when test="$nodeName='kvz_light_control'">
            <xsl:apply-templates mode="kvz_light_control" select="//*[@XlinkId=$XlinkId]"></xsl:apply-templates>
          </xsl:when>
