		...
		<xsl:choose>
			<xsl:when test="$parent_is_system!='false'">
				...
				<xsl:if test="$local_count>0">
					<div class="nav_sub" id="subnavigatie">
						<h3>Subnavigatie</h3>
						<ul>
							<xsl:apply-templates select="/rsistructurenode/treecontextnodes/objectcollection/descendant::object_class[@object_class_name=$parent_is_system]/object_class/object_class/*" mode="zorgatlas_xhtml_link_list">
			...
			<xsl:when test="/rsistructurenode/currentstructurenode/siteconfig/layout/@treemode='tabs'">
				<xsl:if test="($leaf_link_mode='tree' and $childcount_total_active_tab>0) or ($leaf_link_mode='content' and $childcount_parent_nodes_active_tab>0) or $content_count>0">
					<div class="nav_sub" id="subnavigatie">
						<h3>Subnavigatie</h3>
						<ul>
							<xsl:apply-templates select="/rsistructurenode/treecontextnodes/objectcollection/descendant::root/object_class/*" mode="zorgatlas_xhtml_link_list">
								<xsl:with-param name="classname" select="''"></xsl:with-param>
								<xsl:with-param name="classname_open" select="'open'"></xsl:with-param>
								<xsl:with-param name="classname_active">active <xsl:if test="$content_count_parent_nodes>0"> open</xsl:if></xsl:with-param>
								<xsl:with-param name="leaf_link_mode" select="$leaf_link_mode"></xsl:with-param>
			...
			<xsl:otherwise>
				<xsl:if test="($leaf_link_mode='tree' and $childcount_total>0) or ($leaf_link_mode='content' and $childcount_parent_nodes>0) or $content_count>0">
					<xsl:if test="$object_type_id!='22'">
					<div class="nav_sub" id="subnavigatie">
						<h3>Subnavigatie</h3>
						<ul>
							<xsl:apply-templates select="/rsistructurenode/treecontextnodes/objectcollection/root/object_class" mode="zorgatlas_xhtml_link_list">
								<xsl:with-param name="classname" select="''"></xsl:with-param>
								<xsl:with-param name="classname_open" select="'open'"></xsl:with-param>
								<xsl:with-param name="classname_active">active <xsl:if test="$content_count_parent_nodes>0"> open</xsl:if></xsl:with-param>
								<xsl:with-param name="leaf_link_mode" select="$leaf_link_mode"></xsl:with-param>
