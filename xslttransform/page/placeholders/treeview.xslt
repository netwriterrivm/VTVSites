...
<xsl:stylesheet version="1.0"  exclude-result-prefixes="asp ucidb" xmlns:ucidb="http://www.piet.nl"  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:asp="urn:schemas-microsoft-com:datatypes">
	...
	<xsl:template name="CPHTreeView">
		...
		<xsl:param name="content_count_parent_nodes" select="count(/rsistructurenode/treecontextnodes/objectcollection/descendant::*[@structure_id=$structure_id]/object_class[@status_id='23' and @object_type_id='23']) + count(/rsistructurenode/treecontextnodes/objectcollection/descendant::*[@structure_id=$structure_id]/object_class[@status_id='29' and @object_type_id='23']) + count(/rsistructurenode/treecontextnodes/objectcollection/descendant::*[@structure_id=$structure_id]/code_file)"></xsl:param>
		...
		<!--<xsl:param name="CPHTreeView_filled">
			<xsl:choose>
				...
				<xsl:when  test ="/rsistructurenode/currentstructurenode/siteconfig/layout/@treemode!='classic'">true</xsl:when>
		...
		<xsl:param name="display_item"><xsl:apply-templates mode="display_item" select="/rsistructurenode/treecontextnodes/objectcollection/descendant::*[@structure_id=$structure_id]"></xsl:apply-templates></xsl:param>
					
		<xsl:choose>
			<xsl:when test="$parent_is_system!='false' and $releaseoutputmode!='1'">
				<!-- blokken die onder systeem onderdelen vallen worden anders gerenderd-->
				<xsl:variable name="local_count" select="count(/rsistructurenode/treecontextnodes/objectcollection/descendant::object_class[@structure_id=$structure_id]/object_class[@status_id='23' and @object_type_id='23']|/rsistructurenode/treecontextnodes/objectcollection/descendant::object_class[@structure_id=$structure_id]/object_class[@status_id='29' and @object_type_id='23']|/rsistructurenode/treecontextnodes/objectcollection/descendant::object_class[@structure_id=$structure_id]/code_file)"></xsl:variable>
				<!--<xsl:if test="$local_count>0">-->
					<div class="nav_sub" id="subnavigatie">
						<h3>
							<xsl:choose>
								<xsl:when test="/rsistructurenode/currentstructurenode/root/@language_id='2'">Subnavigation</xsl:when>
								<xsl:otherwise>Subnavigatie</xsl:otherwise>
							</xsl:choose>
						</h3>
						<ul>					
							<xsl:apply-templates select="/rsistructurenode/treecontextnodes/objectcollection/descendant::*[@structure_id=$structure_id]/ancestor-or-self::*[@structure_level='2']" mode="treeview">
								<xsl:with-param name="classname" select="''"></xsl:with-param>
								<xsl:with-param name="classname_open" select="'open'"></xsl:with-param>
								<xsl:with-param name="classname_active">active <xsl:if test="$content_count_parent_nodes>0"> open</xsl:if></xsl:with-param>
								<xsl:with-param name="classname_childs" select="'childs'"></xsl:with-param>
	...
	<!-- Content blokken linkerberm-->
			...
			<xsl:when test="/rsistructurenode/currentstructurenode/siteconfig/layout/@treemode='tabs'">
				...
				<xsl:if test="($leaf_link_mode='tree' and $childcount_total_active_tab>0) or ($leaf_link_mode='content' and $childcount_parent_nodes_active_tab>0) or $content_count>0 or (/rsistructurenode/currentstructurenode/siteconfig/layout/@content_page_mode='tree' and ($object_type_id='2' or $object_type_id='5' or $object_type_id='6' or $object_type_id='8'))">
					<div class="nav_sub" id="subnavigatie">
						<h3>
							<xsl:choose>
								<xsl:when test="/rsistructurenode/currentstructurenode/root/@language_id='2'">Subnavigation</xsl:when>
	...
	<!-- Content blokken linkerberm-->
			...
			<xsl:otherwise>
				<xsl:if test="($leaf_link_mode='tree' and $childcount_total>0) or ($leaf_link_mode='content' and $childcount_parent_nodes>0) or $content_count>0  or (/rsistructurenode/currentstructurenode/siteconfig/layout/@content_page_mode='tree' and ($object_type_id='2' or $object_type_id='5' or $object_type_id='6' or $object_type_id='8'))">
						<xsl:if test="$object_type_id!='22'  or /rsistructurenode/currentstructurenode/siteconfig/layout/@treemode='classic'">
					<div class="nav_sub" id="subnavigatie">
						<h3>
							<xsl:choose>
								<xsl:when test="/rsistructurenode/currentstructurenode/root/@language_id='2'">Subnavigation3</xsl:when>
								<xsl:otherwise>Subnavigatie</xsl:otherwise>
							</xsl:choose>
						</h3>
						<ul>
							<xsl:choose>
							<xsl:when test="$releaseoutputmode='1' or /rsistructurenode/currentstructurenode/siteconfig/layout/@treemode='classic'"><xsl:apply-templates select="/rsistructurenode/treecontextnodes/objectcollection/root" mode="treeview">
								<xsl:with-param name="classname" select="''"></xsl:with-param>
								<xsl:with-param name="classname_open" select="'open'"></xsl:with-param>
								<xsl:with-param name="classname_active">active <xsl:if test="$content_count_parent_nodes>0"> open</xsl:if></xsl:with-param>
								<xsl:with-param name="classname_childs" select="'childs'"></xsl:with-param>
								...
								<xsl:otherwise><xsl:apply-templates select="/rsistructurenode/treecontextnodes/objectcollection/root/*" mode="treeview">
								<xsl:with-param name="classname" select="''"></xsl:with-param>
								<xsl:with-param name="classname_open" select="'open'"></xsl:with-param>
								<xsl:with-param name="classname_active">active <xsl:if test="$content_count_parent_nodes>0"> open</xsl:if></xsl:with-param>
								<xsl:with-param name="classname_childs" select="'childs'"></xsl:with-param>
	...
	<xsl:template name="CPHTreeView_filled">
		...
		<xsl:choose>
			...
			<xsl:otherwise><xsl:if test="($leaf_link_mode='tree' and $childcount_total>0) or ($leaf_link_mode='content' and $childcount_parent_nodes>0) or $content_count>0">true</xsl:if></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
</xsl:stylesheet>
