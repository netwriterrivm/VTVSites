...
$(document).ready(function () {
    /*Added function by W.Corts to make it government webguideline proof. Add <div id="twitterloketgezondleven"><a href="#" id="wrl_twitlink" title="Naar de twitterpagina">Naar de twitter pagina van het loket</a><!--DNC--></div> to the page.*/
    $('.wrl_twitlink').remove();
    /*End code W.Corts*/
});
...
; (function ($) {
    ...
    $.fn.twitterSearch = function (options) {
        ...
        return this.each(function () {
            ...
            if (opts.bird) {
                var bird = '<img class="twitterSearchBird" src="' + opts.birdSrc + '" />';
            }
            text = opts.titleLink ? ('<a class="gototwitter" href="' + opts.titleLink + '">' + bird + '<span>' + opts.title + '</span></a>') : ('<span>' + opts.title + '</span>');
            $text = $(text);
            if (opts.titleLink)
    ...
    $.fn.twitterSearch.formatter = function (json, opts) {
        ...
        //str += '<img class="twitterSearchProfileImg" src="http://www.loketgezondleven.nl/object_binary/o13365.jpg" />'; //JB: svp geen site specifieke code in een script dat door meerdere sites gebruikt wordt!!!!

        // standaard url, niet meer naar bericht, dit ging heel vaak fout.        
        // url configureerbaar via site-specifieke js (bijv. loketgezondleven.js).

        if (opts.userLink) {
            sLinkUrl = opts.userLink;
        }
        else {
            sLinkUrl = 'http://www.twitter.com/' + json.from_user;
        }

        str += '<div><span class="twitterSearchUser"><a href="' + sLinkUrl + '">'
    ...
    $.fn.twitterSearch.defaults = {
        ...
        timeout: 10000, 			// delay betweet tweet scroll
        ...
        css: {
            ...
            bird: { width: '50px', height: '20px', left: '-30px', top: '-20px', border: 'none' },
            container: { overflow: 'hidden', backgroundColor: '#f2d9e7' },
            fail: { background: '#f2d9e7 url(http://cloud.github.com/downloads/malsup/twitter/failwhale.png) no-repeat 50% 50%', height: '100%', padding: '10px' },
            frame: { border: '10px solid #f2d9e7' },
            tweet: { padding: '5px 10px', clear: 'left' },
            img: { 'float': 'left', margin: '5px', width: '48px', height: '48px' },
            loading: { padding: '20px', textAlign: 'center', color: '#888' },
            text: {},
            time: { fontSize: '9px', color: '#888' },
            title: { backgroundColor: '#f2d9e7', margin: 0, padding: '0 0 5px 0', textAlign: 'center', fontWeight: 'bold', fontSize: 'large', position: 'relative' },
